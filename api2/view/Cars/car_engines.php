...
<body>
      <div class="container">
            ...
            <div class="crud__main">
                  <form method="post" action="index.php?page=handlers&table=cars&sub=engines">
                        <div class="form-group">
                              <input type="submit" value="Ajouter">
                              <input type="text" placeholder="Nom de la motorisation" name="car_engines_name" required>
                              <input type="hidden" name="car_engines_id" value="0">
                        </div>
                  </form>
                  <table class="crud__table">
                        <tr>
                              <td class=" crud__table-cell crud__table-cell-header ">Actions</td>
                              <?php foreach (Car_engines::$array_accepted_key as $field):
                                    if ($field['crud_show']) { ?>
                        ...
                        <?php foreach ($engines as $div):
                              $div = new Car_engines($div) ?>

                              <tr>
                                    <td class="crud__table-cell">
                                          <div class="crud__table-cta">
                                                <a href="index.php?page=handlers&table=cars&sub=engines&remove&id=<?= $div->id() ?>"
                                                      class="crud__table-btn crud__table-btn--delete">X</a>
                                          </div>
                                    </td>
                                    <?php foreach (Car_engines::$array_accepted_key as $key => $value):
                                          if ($value['crud_show']) {
                                                $method = str_replace('car_engines_', '', $key) ?>
