      ...
      <div class="container">
            ...
            <div class="crud__header">
                  <a href="index.php?page=accounts" class="crud__back">
                        <\Retour </a>
                              <h2><?= ucfirst($title) ?></h2>
                              <a href="index.php?page=process&table=<?= $table ?>&mode=remove&id=<?= $recordset['accounts'][$table . '_id'] ?>" class="crud__table-btn crud__table-btn--delete">Supprimer le compte</a>
                  ...
                  <div class="form__group">
                        <form method="post" action="index.php?page=process&table=<?= $table ?>&mode=save">
                              ...
                              if ($value['detail_show']) { ?>
                                    ...
                                    <div class="form-group">
                                          <form method="post" action="index.php?page=process&table=accounts_preferences&mode=save">
                                                <h5>Préférences</h5>
                                                <?php foreach ($recordset['preferences'] as $preference): ?>
                                          ...
                                          <?= in_array($preference, $recordset['accountPreferences']) ? 'checked' : '' ?>>
                                          <label class="form-check-label" for="preference_<?= $preference['preferences_id'] ?>">
                                                <?= htmlspecialchars($preference['preferences_name']) ?>
                                          </label>
                                    </div>
                                    <?php endforeach; ?>
                                    <input type="hidden" name="accounts_id" value="<?= $recordset['accounts']['accounts_id'] ?>">
